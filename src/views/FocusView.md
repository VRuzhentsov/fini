...
- Focus enter count is a persisted transition metric. `get_active_focus` compares the resolved quest id with the last recorded Focus id and increments `quest.focus_enter_count` only when the id changes, so refreshes and polling do not inflate it.
