...
const grouped = computed(() => groupHistoryItems(history.value));
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <p v-if="!grouped.rows.length" class="text-sm opacity-40">No completed or abandoned quests yet.</p>
    <QuestList v-else :quests="grouped.rows" :group-children-by-id="grouped.groupChildrenById" />
  </div>
</template>
