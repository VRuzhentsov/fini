...
import { channelRowState, transportStatusText } from "../../utils/transportStatusCodes";
...
function deviceSummary(device: PairedDevice): string {
  const statuses = deviceStore.getTransportStatuses(device.peer_device_id);
  ...
  const reason = candidate?.state.code
    ? transportStatusText(candidate.state.code, device.display_name)
    : null;
