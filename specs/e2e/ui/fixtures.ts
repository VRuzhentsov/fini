 ...
 * - When FINI_BINARY points at a prebuilt e2e binary, no `tauri dev` is started,
...
const tauriCommand = process.env.FINI_BINARY
  ? (headful
      ? `env ${envFlags} ${process.env.FINI_BINARY} app`
      : `xvfb-run -a env ${envFlags} ${process.env.FINI_BINARY} app`)
  : `env ${envFlags} npx tauri dev --features e2e-testing -- -- app`;
