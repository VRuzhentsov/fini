-- SQLite cannot DROP COLUMN, so recreate quests without series_id/period_key
CREATE TABLE quests_backup AS SELECT
    id, space_id, title, description, status, energy, priority, pinned,
    due, due_time, repeat_rule, completed_at, set_main_at,
    reminder_triggered_at, order_rank, created_at, updated_at
FROM quests;
...
INSERT INTO quests SELECT * FROM quests_backup;
DROP TABLE quests_backup;
