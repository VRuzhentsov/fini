...
/// immediately re-runs primary-transport selection (`refresh_primary`) so
/// the UI reflects it without waiting for a reconnect -- both transports
...
/// `"network"`/`"bluetooth"` (`transport_kind_to_preference_string`'s
/// stored form), or `None` for pure automatic primary selection. Both
/// transports dial/connect unconditionally regardless of this pin (ADR-0003
/// revision) -- it's consulted only by `DeviceConnectionState::
/// recompute_primary_locked`, to decide which already-connected transport
...
/// Which transport (if any) is currently primary for a peer. Debug/test
/// surface proving per-transport claiming end-to-end through the real app
...
mod tests {
    ...
    /// can no longer be honored, stranding the pair on no transport at all.
    #[test]
