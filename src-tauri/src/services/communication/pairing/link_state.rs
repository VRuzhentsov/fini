//! The owned state machine for one peer's link on one channel (ADR-0005).
//!
...
//! (`channel::ble::should_dial_peer`), so the caller owns that decision and
...
/// Matches `channel::ble::AUTO_RETRY_WINDOW`, whose own doc comment records
...
/// One peer's link on one channel.
///
...
pub(super) enum LinkEffect {
    /// Close the channel link and release the session slot. Emitted only
...
impl LinkState {
    ...
    pub(super) fn apply(&self, event: LinkEvent, now: Instant) -> (LinkState, Vec<LinkEffect>) {
        ...
        match (self, event) {
            ...
            // amber while a dead session kept the channel's only slot, and
