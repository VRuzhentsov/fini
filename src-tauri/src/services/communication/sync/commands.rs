...
/// `pairing::runtime::DEVICE_ID_KEY`) so the checklist merge push-back can stamp an
...
///
/// It was 3s, chosen when the only channel was a network socket on mains
...
pub fn space_sync_tick_impl(
    ...
    for peer_device_id in &peer_ids {
        ...
        for event in pending.iter() {
            ...
            // (or whose channel can't drain as fast as this tick fires)
            ...
            // limited resends against any channel); BLE's small per-
...
mod tests {
    ...
    async fn space_sync_tick_does_not_replay_mapping_snapshot_when_session_appears() {
        ...
        while let Ok(sent) = rx.try_recv() {
            if matches!(
                sent,
                crate::services::communication::sync::types::SessionCommand::Forward(
                    PeerFrame::SpaceMappingUpdate { .. }
                )
            ) {
