...
ENV FINI_E2E_SOCKET_DIR=/var/run/fini-e2e \
    ...
    FINI_BLE_MOCK_BROKER_BINARY=/usr/local/bin/ble-mock-broker \
    FINI_E2E_CONTAINER_RUNNER=1 \
    TZ=UTC

CMD ["sh", "./scripts/e2e-runner.sh"]

# Local Android build environment: JDK 17 plus Node, for hosts whose system JDK
# is too new for the Gradle version this project pins -- Gradle's bundled Kotlin
# DSL compiler fails to parse some newer JDK version strings before it ever
# reads our build files. The concrete failure, on a host whose `java` resolves
# to the flatpak Android Studio's bundled JBR (which auto-updates itself):
#
#     A problem occurred configuring project ':buildSrc'.
#     > 25.0.2
#
# Unlike every stage above, nothing is COPYed in: the repo, Android SDK/NDK and
# the cargo/gradle caches are all bind-mounted from the host at run time (see
# ANDROID_BUILD_RUN in the Makefile), so this stage stays tiny and holds no
# project state. `adb` deliberately stays on the host, where the USB device and
# adb server are -- reaching them from inside would mean handing the container
# far more device access than a build needs.
#
# Built on its own: make android-build-image
FROM docker.io/library/eclipse-temurin:17-jdk AS android-build

# build-essential is required even though nothing here targets the host: cargo
# compiles each crate's *build script* for the host triple first, and those need
# a working host linker (`cc`). Without it the Android build fails at
# "linker `cc` not found" while compiling build scripts, long before it reaches
# any Android cross-compilation.
RUN apt-get update \
